...
class DirectionControlSystem:
    ...
    def __init__(
        ...
        # Track whether we actually have a PSoC connection
        self.psoc_connected = False
        self.psoc_warning_printed = False
    ...
    def start(self):
        ...
        self.psoc_connected = self.psoc_communicator.connect()
        if not self.psoc_connected:
            print(
                "Warning: Could not connect to PSoC. "
                "Continuing with angle estimation ONLY (no car control)."
            )
    ...
    def run_control_loop(self):
        ...
        while self.running:
            ...
            # Estimate direction
            angle = self.direction_estimator.estimate_angle(
            ...
            smoothed_angle = self.direction_estimator.smooth_angle(
                self.angle_history
            )
            ...
            if current_time - last_update >= self.update_interval:
                if smoothed_angle is not None:
                    ...
                    # Only send to PSoC if we actually connected
                    if self.psoc_connected:
                        self.psoc_communicator.send_angle_simple(smoothed_angle)
                    elif not self.psoc_warning_printed:
                        # Print this once so we don't spam the console
                        print(
                            "Note: running without PSoC connection; "
                            "angles are printed but not sent to the car."
                        )
                        self.psoc_warning_printed = True

                    # Always print the angle estimate
                    if angle is not None:
                        print(
                            f"Angle (smoothed): {smoothed_angle:7.2f}° | "
                            f"Raw: {angle:7.2f}°"
                        )
                    else:
                        print(
                            f"Angle (smoothed): {smoothed_angle:7.2f}° | Raw: N/A"
                        )
                else:
                    # Keep last valid angle or indicate no signal
                    if self.last_angle != 0:
                        print(
                            f"No strong signal. Last angle: {self.last_angle:.2f}°"
                        )
                    else:
                        print("No signal detected yet.")
