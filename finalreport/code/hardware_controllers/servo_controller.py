...
DESIGN OVERVIEW:
This module controls the steering servo motor using PWM signals.
The servo provides ±45° steering range for vehicle navigation.

KEY DESIGN DECISIONS:
1. PWM Frequency: 50Hz chosen for servo motor control
   - Standard servo motors operate at 50Hz (20ms period)
   - This frequency provides smooth, responsive steering control
   - Different from motor frequency (40Hz) due to different requirements
   
2. Duty Cycle Range: 89.318% to 95.422% for ±45° steering
   - Center position: 92.600% duty cycle (0° steering)
   - Left maximum: 95.422% duty cycle (-45° steering)
   - Right maximum: 89.318% duty cycle (+45° steering)
   - Linear mapping: angle = (duty - center) / (max - center) * 45°
   
3. Angle Clamping: ±45° maximum steering angle
   - Prevents over-steering which could damage servo or cause instability
   - Provides safe, controlled turning radius
   
4. Servo Selection: Standard RC servo motor
   - Provides precise angular control
   - Fast response time (~100ms for full range)
   - Reliable operation with PWM control
"""
